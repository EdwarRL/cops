...
*;
      ...
      eMTL_CI(uni_t, fuel, p)$[L_up_minUni(uni_t,fuel) gt 0 and ord(p) le L_up_minUni(uni_t,fuel) and uni_fuel(uni_t,fuel) and 1=1 and (not pCasoPID)] ..
         vCommitmentUni(uni_t,p)=e= onoff_t0Uni(uni_t)
      ;

      eMTFL_CI(uni_t, fuel, p)$[L_down_minUni(uni_t,fuel) gt 0 and ord(p) le L_down_minUni(uni_t,fuel) and uni_fuel(uni_t,fuel) and 1=1 and (not pCasoPID)] ..
         vCommitmentUniMC(uni_t,p)=e= onoff_t0Uni(uni_t)
      ;

      eMTL(uni_t, fuel, p)$ (uni_fuel(uni_t,fuel) and (not pCasoPID)) ..
   ...
   * eMTFL(uni_t, fuel, p)$ (uni_fuel(uni_t,fuel) and (sameas(uni_t,'FLORES_1_GAS') or sameas(uni_t,'FLORES_1_VAPOR'))) ..
         eMTFL(uni_t, fuel, p)$ (uni_fuel(uni_t,fuel) and (not pCasoPID)) ..
      ...
      eMaxStartupxDay(uni_t, d)$((not pCasoPID))  ..
         sum[p$pd(p,d), vStartUpUni(uni_t,p)] =l= pStartUpxDay(uni_t)
      ;

eMaxStartupxDayMC(uni_t,d)$((not pCasoPID))  ..
...
eUniCombGTSteam(pltt_cc,p)$(bMC=1)..
...
eCSThmPltUpRamps (pltt_cs,p)$(not pCasoPID)..
...
eCSThmPltDRRamps(pltt_cs,p)$(not pCasoPID) ..
...
eUpRampMod21CS(pltt_cs,p)$[(ord(p) lt card(p)) and (not pltt_m3(pltt_cs)) and (not pCasoPID)] ..
   vGenTrmPltM2(pltt_cs,p+1) =l= vGenTrmPltM2(pltt_cs,p) + vDeltaU(pltt_cs,p) + vGenTrmPltURM1(pltt_cs,p) + pM*[vbUpMT (pltt_cs,p)]
;

eUpRampMod22CS(pltt_cs,p)$(not pltt_m3(pltt_cs) and (not pCasoPID))                     ..
   vDeltaU(pltt_cs,p)
  =l= sum[(conf,r)$(pltt_conf(pltt_cs,conf) and (ord(conf)=pThmConf(pltt_cs)) ),pUpRateRmp( pltt_cs,conf,r)*vbUR(pltt_cs,r,p)]
;

eUpRampMod23CS(pltt_cs,p)$(not pltt_m3(pltt_cs) and (not pCasoPID))                     ..
   sum[r,vbUR(pltt_cs,r,p)]
  =l= 1
;

eUpRampMod24CS(pltt_cs,p)$[(ord(p) lt card(p)) and (not pltt_m3(pltt_cs)) and (not pCasoPID)] ..
...
eUpRampMod25CS(pltt_cs,p)$[(ord(p) lt card(p)) and (not pltt_m3(pltt_cs)) and (not pCasoPID)] ..
  vGenTrmPltM2(pltt_cs,p) + vGenTrmPltURM1(pltt_cs,p) - pM*[1 - vCommitTrmPlt(pltt_cs,p+1)] - pM*[vbUpMT (pltt_cs,p)]
...
eUpRampMod21CS_CI(pltt_cs,p)$[(ord(p) = 1) and (not pltt_m3(pltt_cs)) and bCI=1 and (not pCasoPID)] ..
   vGenTrmPltM2(pltt_cs,p) =l= vGenTrmPltM2_CI(pltt_cs) + vDeltaU_CI(pltt_cs) + vGenTrmPltURM1_CI(pltt_cs) + pM*[vbUpMT_CI(pltt_cs)]
;

eUpRampMod22CS_CI(pltt_cs)$(not pltt_m3(pltt_cs) and bCI=1 and (not pCasoPID))                     ..
   vDeltaU_CI(pltt_cs)
  =l= sum[(conf,r)$(pltt_conf(pltt_cs,conf) and (ord(conf)=pConfPltIni(pltt_cs)) ),pUpRateRmp( pltt_cs,conf,r)*vbUR_CI(pltt_cs,r)]
;

eUpRampMod23CS_CI(pltt_cs)$(not pltt_m3(pltt_cs) and bCI=1 and (not pCasoPID))                     ..
   sum[r,vbUR_CI(pltt_cs,r)]
  =l= 1
;

eUpRampMod24CS_CI(pltt_cs,p)$[(ord(p) = 1) and (not pltt_m3(pltt_cs)) and bCI=1 and (not pCasoPID)] ..
...
eUpRampMod25CS_CI(pltt_cs,p)$[(ord(p) = 1) and (not pltt_m3(pltt_cs)) and bCI=1 and (not pCasoPID)] ..
  vGenTrmPltM2_CI(pltt_cs) + vGenTrmPltURM1_CI(pltt_cs) - pM*[1 - vCommitTrmPlt(pltt_cs,p)] - pM*[vbUpMT_CI (pltt_cs)]
...
eDwnRampMod21CS (pltt_cs,p)$[(ord(p) lt card(p)) and (not pltt_m3(pltt_cs)) and banMod2CS=1 and (not pCasoPID)] ..
   vGenTrmPltM2(pltt_cs,p+1) + vGenTrmPltDRM1(pltt_cs,p+1) =g= vGenTrmPltM2(pltt_cs,p) - vDeltaD(pltt_cs,p) - pM*[vbDownMT (pltt_cs,p)]
;

eDwnRampMod22CS (pltt_cs,p)$(not pltt_m3(pltt_cs) and banMod2CS=1 and (not pCasoPID))                                                              ..
   vDeltaD(pltt_cs,p)
   =l= sum[(conf,r)$(pltt_conf(pltt_cs,conf) and (ord(conf)=pThmConf(pltt_cs)) ),pDownRateRmp(pltt_cs,conf,r)*vbDR(pltt_cs,r,p)]
;

eDwnRampMod23CS (pltt_cs,p)$(not pltt_m3(pltt_cs) and banMod2CS=1 and (not pCasoPID))                                                              ..
  sum[r,vbDR(pltt_cs,r,p)]
 =l= 1
;

eDwnRampMod24CS (pltt_cs,p)$[(ord(p) lt card(p)) and (not pltt_m3(pltt_cs)) and banMod2CS=1 and (not pCasoPID)]                                         ..
...
eDwnRampMod25CS (pltt_cs,p)$[(ord(p) lt card(p)) and (not pltt_m3(pltt_cs)) and banMod2CS=1 and (not pCasoPID)]                                         ..
   vGenTrmPltM2(pltt_cs,p) - pM*[1 - vCommitTrmPlt(pltt_cs,p+1)]
...
eDwnRampMod21CS_CI (pltt_cs,p)$[(ord(p) =1) and (not pltt_m3(pltt_cs)) and (pDispThmPlt(pltt_cs,p)>0) and (not pCasoPID)] ..
   vGenTrmPltM2(pltt_cs,p) + vGenTrmPltDRM1(pltt_cs,p) =g= vGenTrmPltM2_CI(pltt_cs) - vDeltaD_CI(pltt_cs) - pM*[vbDownMT_CI(pltt_cs)]
;

eDwnRampMod22CS_CI (pltt_cs,p)$((ord(p) =1) and (not pltt_m3(pltt_cs)) and (pDispThmPlt(pltt_cs,p)>0) and (not pCasoPID))                                                              ..
   vDeltaD_CI(pltt_cs)
   =l= sum[(conf,r)$(pltt_conf(pltt_cs,conf) and (ord(conf)=pConfPltIni(pltt_cs)) ),pDownRateRmp(pltt_cs,conf,r)*vbDR_CI(pltt_cs,r)]
;

eDwnRampMod23CS_CI (pltt_cs,p)$( (ord(p) =1) and (not pltt_m3(pltt_cs)) and (pDispThmPlt(pltt_cs,p)>0) and (not pCasoPID))                                                              ..
  sum[r,vbDR_CI(pltt_cs,r)]
 =l= 1
;

eDwnRampMod24CS_CI (pltt_cs,p)$[(ord(p)=1) and (not pltt_m3(pltt_cs)) and (pDispThmPlt(pltt_cs,p)>0) and (not pCasoPID)]                                         ..
...
eDwnRampMod25CS_CI (pltt_cs,p)$[(ord(p)=1) and (not pltt_m3(pltt_cs)) and (pDispThmPlt(pltt_cs,p)>0) and (not pCasoPID)]                                         ..
   vGenTrmPltM2_CI(pltt_cs) - pM*[1 - vCommitTrmPlt(pltt_cs,p)]
...
eUpRampMod31CS(pltt_cs,p,l,ll,conf)$[(ord(p) gt 1) and pltt_m3(pltt_cs) and (ord(l) eq 1) and (ord(ll) eq 2) and pltt_conf(pltt_cs,conf) and (ord(conf)=pThmConf(pltt_cs)) and (not pCasoPID)]..
...
eUpRampMod32CS(pltt_cs,p,l,ll,conf)$[(ord(p) eq 1) and pltt_m3(pltt_cs) and (ord(l) eq 1) and (ord(ll) eq 2) and pltt_conf(pltt_cs,conf) and (ord(conf)=pThmConf(pltt_cs)) and (not pCasoPID)]..
  pUpRampMod3(pltt_cs,conf,l)*vGenTrmPltM2(pltt_cs,p)  - pUpRampMod3(pltt_cs,conf,ll)*pGenPltiIni(pltt_cs)
  =l= pURMod3(pltt_cs,conf)
;

* Down ramp model 3
eDwnRampMod31CS(pltt_cs,p,l,ll,conf)$[(ord(p) gt 1) and pltt_m3(pltt_cs) and (ord(l) eq 1) and (ord(ll) eq 2) and pltt_conf(pltt_cs,conf) and (ord(conf)=pThmConf(pltt_cs)) and (not pCasoPID)]..
...
eDwnRampMod32CS(pltt_cs,p,l,ll,conf)$[(ord(p) eq 1) and pltt_m3(pltt_cs) and (ord(l) eq 1) and (ord(ll) eq 2) and pltt_conf(pltt_cs,conf) and (ord(conf)=pThmConf(pltt_cs)) and (not pCasoPID)]..
...
eMinUnitZones(zn,p)$( pUnitZoneReq(zn,p) > 0 and pZoneType('uni',zn)=1 and banZn=1 and (not pCasoPID))..
   sum[uni$(pWeightUnitZone(uni,p,zn)>0),pWeightUnitZone(uni,p,zn)*vCommitmentUni(uni,p)] =g= pUnitZoneReq(zn,p)
;

eMinMWZones(zn,p)$( pMWminZoneReq(zn,p) > 0 and pZoneType('min',zn)=1 and banZn=1 and not sameas(zn,'Z_MinTer') and (not pCasoPID))..
   sum[uni_t$(pWeightUnitZone(uni_t,p,zn)>0), vGenUniM2(uni_t,p) ] + sum[uni_h$(pWeightUnitZone(uni_h,p,zn)>0), vGenUni(uni_h,p) ]
   + sum[uni_v$(pWeightUnitZone(uni_v,p,zn)>0), vGenUni(uni_v,p) ]
   =g= pMWminZoneReq(zn,p)
;

eMaxMWZones(zn,p)$( pMWmaxZoneReq(zn,p) > 0 and pMWmaxZoneReq(zn,p) < 9999 and pZoneType('max',zn)=1 and banZn=1 and (not pCasoPID))..
   sum[uni$(pWeightUnitZone(uni,p,zn)>0),vGenUni(uni,p)] =l= pMWmaxZoneReq(zn,p)
;

eMinMWZonesDay(zn)$(pZoneType('min',zn)=1 and sameas(zn,'Z_MinTer') and (pTermicaDia>0)  and banZnD=1 and (not pCasoPID))..
   sum[(uni_t,p)$(pWeightUnitZone(uni_t,p,zn)>0), vGenUniM2(uni_t,p) ] + sum[(uni_h,p)$(pWeightUnitZone(uni_h,p,zn)>0), vGenUni(uni_h,p) ]
   + sum[(uni_v,p)$(pWeightUnitZone(uni_v,p,zn)>0), vGenUni(uni_v,p) ]
   =g= pTermicaDia * 1000
;

eAreaImportLimit(ar,p)$(pArImportLim(ar,p)<9999 and banZn=1 and (not pCasoPID))..
   sum[sba$(ar_sba(ar,sba)),pDemandSba(p,sba)]-
   (sum[(uni_t,sba)$(uni_sba(uni_t,sba) and ar_sba(ar,sba)), vGenUniM2(uni_t,p)]
   +sum[(uni_h,sba)$(uni_sba(uni_h,sba) and ar_sba(ar,sba)),vGenUni(uni_h,p)]
   +sum[(uni_v,sba)$(uni_sba(uni_v,sba) and ar_sba(ar,sba)),vGenUni(uni_v,p)])
   =l= pArImportLim(ar,p)
;

eMinGenPlt(plt,p)$(pMinGenPlt(plt,p)>0 and banZn=1 and (not pCasoPID))..
   vGenPlt(plt,p)=g=pMinGenPlt(plt,p)
;

eMaxGenPlt(plt,p)$(pMaxGenPlt(plt,p)<9999 and pMaxGenPlt(plt,p)>0  and banZn=1 and (not pCasoPID))..
...
eEqMT1CS(pltt_cs,p)$[(not pltt_m3(pltt_cs)) and (not pCasoPID)]..
   vGenTrmPltM2(pltt_cs,p) =l= pTrmCSMinConf(pltt_cs)  + pM*[1 - vbMT(pltt_cs,p)]
;

eEqMT2CS(pltt_cs,p)$[(not pltt_m3(pltt_cs)) and (not pCasoPID)]..
   vGenTrmPltM2(pltt_cs,p) =g= pTrmCSMinConf(pltt_cs)*vbMT(pltt_cs,p)
;

eEqMT3CS(pltt_cs,p)$[(not pltt_m3(pltt_cs)) and (not pCasoPID)]..
...
eEqMT1CS_CI(pltt_cs)$[(not pltt_m3(pltt_cs)) and (not pCasoPID)]..
   vGenTrmPltM2_CI(pltt_cs) =l= pTrmCSMinConf(pltt_cs)  + pM*[1 - vbMT_CI(pltt_cs)]
;

eEqMT2CS_CI(pltt_cs)$[(not pltt_m3(pltt_cs)) and (not pCasoPID)]..
   vGenTrmPltM2_CI(pltt_cs) =g= pTrmCSMinConf(pltt_cs)*vbMT_CI(pltt_cs)
;

eEqMT3CS_CI(pltt_cs)$[(not pltt_m3(pltt_cs)) and (not pCasoPID)]..
   vbMT_CI(pltt_cs) =g=
...
eMenvCommitDn(plt_t,p)$[(ord(p) lt card(p)) and (not pltt_m3(plt_t)) and banAux2=1 and (not pCasoPID)]..
   vbDownMT (plt_t,p) =l= 1- vCommitTrmPlt(plt_t,p+1)
;

eMenvMTDn(plt_t,p)$[(not pltt_m3(plt_t)) and banAux2=1 and (not pCasoPID)]..
   vbDownMT(plt_t,p) =l= vbMT(plt_t,p)
;

eDownMTDn(plt_t,p)$[(ord(p) lt card(p)) and (not pltt_m3(plt_t)) and banAux2=1 and (not pCasoPID)]..
   vbDownMT (plt_t,p) =g= [1-vCommitTrmPlt(plt_t,p+1)] + vbMT(plt_t,p) - 1
;

// Condición inicial para la bandera de bajada
eMenvCommitDn_CI(plt_t,p)$[(ord(p) =1) and (not pltt_m3(plt_t)) and bCI=1 and (not pCasoPID)]..
   vbDownMT_CI(plt_t) =l= 1- vCommitTrmPlt(plt_t,p)
;

eMenvMTDn_CI(plt_t)$[(not pltt_m3(plt_t)) and bCI=1 and (not pCasoPID)]..
   vbDownMT_CI(plt_t) =l= vbMT_CI(plt_t)
;

eDownMTDn_CI(plt_t,p)$[(ord(p) =1) and (not pltt_m3(plt_t)) and bCI=1 and (not pCasoPID)]..
...
eMenvCommitUp(plt_t,p)$[(not pltt_m3(plt_t)) and banAux2=1 and (not pCasoPID)]..
   vbUpMT (plt_t,p) =l= 1- vCommitTrmPlt(plt_t,p)
;

eMenvMTUp(plt_t,p)$[(ord(p) lt card(p)) and (not pltt_m3(plt_t)) and banAux2=1 and (not pCasoPID)]..
   vbUpMT(plt_t,p) =l= vbMT(plt_t,p+1)
;

eDownMTUp(plt_t,p)$[(ord(p) lt card(p)) and (not pltt_m3(plt_t)) and banAux2=1 and (not pCasoPID)]..
   vbUpMT (plt_t,p) =g= [1-vCommitTrmPlt(plt_t,p)] + vbMT(plt_t,p+1) - 1
;

// Condición inicial para la bandera de subida

eMenvCommitUp_CI(plt_t)$[(not pltt_m3(plt_t)) and (not pCasoPID)]..
   vbUpMT_CI (plt_t) =l= 1- 1$[(TypeMod_CI(plt_t)=2) and (pGenPltiIni(plt_t)>0)]
;

eMenvMTUp_CI(plt_t,p)$[(ord(p)=1) and (not pltt_m3(plt_t)) and (not pCasoPID)]..
   vbUpMT_CI(plt_t) =l= vbMT(plt_t,p)
;

eDownMTUp_CI(plt_t,p)$[(ord(p) =1) and (not pltt_m3(plt_t)) and (not pCasoPID)]..
...
parameter
banTULT /1/;
* eTULT(pltt_cc, conf, confAux, tstate,uni_t, fuel, p)$(pltt_conf(pltt_cc, conf) and pltt_conf(pltt_cc, confAux) and plt_uni(pltt_cc,uni_t) 
*          and pTrmCCPltTransition(pltt_cc,conf,confAux)=1 and uni_fuel(uni_t,fuel) and pTrmCCPltTULT(pltt_cc, conf, confAux, tstate)>1 
*          and sameas(tstate,'cold') and sameas(uni_t,'TEBSA_11') and sameas(conf,'c1') and sameas(confAux,'c2') and banTULT=1) ..
*     sum[(pp)$((ord(pp) gt ord(p)) and (ord(pp) le ord(p)+ pTrmCCPltTULT(pltt_cc, conf, confAux, tstate))), vStartUpUniFuelTE(uni_t, fuel, tstate, pp)]
*          -(pM*(1-sum[(pp)$((ord(pp) ge ord(p)) and (ord(pp) le ord(p)+ pTrmCCPltTULT(pltt_cc, conf, confAux, tstate)-1)),vTransitionCC(pltt_cc, conf, confAux, pp)]))
*     =l= vCommitTrmCC(pltt_cc, conf, p)

* eTULT(pltt_cc, conf, confAux, tstate,uni_t, fuel, p, pp)$(pltt_conf(pltt_cc, conf) and pltt_conf(pltt_cc, confAux) and plt_uni(pltt_cc,uni_t) 
...
*          and ((ord(p) + ord(pp)) le card(p)) and sameas(tstate,'cold') and sameas(uni_t,'TEBSA_11') and sameas(conf,'c1') and sameas(confAux,'c2') and banTULT=1) ..
*     vStartUpUniFuelTE(uni_t, fuel, tstate, p + ord(pp))-vTransitionCC(pltt_cc, conf, confAux, p+ord(pp)-1)
*     =l= vCommitTrmCC(pltt_cc, conf, p)
...
* vGenPlt.fx('FLORES_I_CC','9')=65;
* vGenPlt.fx('FLORES_4_CC','20')=0;
...
* vGenPlt.up('TEBSAB_CC','1')=0;
* vGenPlt.up('TEBSAB_CC','24')=0;
* vGenPlt.fx('BARRANQUILLA_3','11')=60;

* vCommitTrmCC.fx('FLORES_4_CC','c2','1')=1;
